...
export async function GET(request: NextRequest) {
    ...
    catch (err) { console.error("GET /api/faqs — DB:", err); return jsonError("Something went wrong", 500); }
    ...
    try {
        ...
        console.error("GET /api/faqs — query:", err);
...
export async function POST(request: NextRequest) {
    ...
    try { await connectToDatabase(); }
    catch (err) { console.error("POST /api/faqs — DB:", err); return jsonError("Something went wrong", 500); }

    try {
        return jsonOk(await FAQ.create(parsed.data), 201);
    } catch (err) {
        console.error("POST /api/faqs — insert:", err);
        return jsonError("Something went wrong", 500);
    }
}
